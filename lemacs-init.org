...
#+BEGIN_SRC emacs-lisp
  (use-package vc
    ...
    (defun lemacs/insert-commit-editmsg ()
      "Insert the contents of .git/COMMIT_EDITMSG into the *vc-log* buffer."
      (interactive)
      (let* ((default-directory (vc-root-dir)) ; Ensure we're in the repository root
             (commit-editmsg-path (expand-file-name ".git/COMMIT_EDITMSG" default-directory)))
        (when (file-exists-p commit-editmsg-path)
          (goto-char (point-max)) ; Move to the end of the buffer
          (insert "Summary:\n\n")
          (insert-file-contents commit-editmsg-path))))

    (add-hook 'log-edit-hook #'lemacs/insert-commit-editmsg)
